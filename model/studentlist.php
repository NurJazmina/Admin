...
if (isset($_POST['status_parent']))
{
  ...
  $bulk = new MongoDB\Driver\BulkWrite(['ordered' => TRUE]);
  $bulk->update(['ConsumerID' => $parent_consumer_id],
  ...
  $bulk->insert([
    'SubRemarks'=>'0',
    'School_id'=>$school_id,
    'Consumer_id'=>$parent_consumer_id,
    'Staff_id'=>$staff_id,
    'Details'=>$detail,
    'Date'=>$date,
    'Status'=>'ACTIVE']);
  ...
  try
  {
    $result = $GoNGetzDatabase->executeBulkWrite('GoNGetzSmartSchool.Parent_Remarks', $bulk, $writeConcern);
  }
  catch (MongoDB\Driver\Exception\BulkWriteException $e)
  {
    $result = $e->getWriteResult();
