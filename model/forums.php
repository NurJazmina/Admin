...
if (isset($_POST['add_forums'])) 
{
  ...
  try
  {
    $result =$GoNGetzDatabase->executeBulkWrite('GoNGetzSmartSchool.Forum', $bulk, $writeConcern);
  }
...
if (isset($_POST['add_comment'])) 
{
  ...
  try
  {
    $result =$GoNGetzDatabase->executeBulkWrite('GoNGetzSmartSchool.ForumComment', $bulk, $writeConcern);
  }
  catch (MongoDB\Driver\Exception\BulkWriteException $e)
  {
    $result = $e->getWriteResult();
...
if (isset($_POST['edit_forum'])) 
{
  ...
  $bulk = new MongoDB\Driver\BulkWrite(['ordered'=>true]);
  $bulk->update(['_id' => new \MongoDB\BSON\ObjectID($forum_id)],
    ['$set' => [
      'Forum'=>$forum,
      'Title'=>$title,
      'Details'=>$detail,
...
if (isset($_POST['delete_forum'])) 
{
  ...
  {
    $bulk = new MongoDB\Driver\BulkWrite(['ordered'=>true]);
    $bulk->delete(['_id'=>new \MongoDB\BSON\ObjectID($forum_id)], ['limit' => 1]);
    $writeConcern = new MongoDB\Driver\WriteConcern(MongoDB\Driver\WriteConcern::MAJORITY, 1000);
    try
    {
      $result =$GoNGetzDatabase->executeBulkWrite('GoNGetzSmartSchool.Forum', $bulk, $writeConcern);
